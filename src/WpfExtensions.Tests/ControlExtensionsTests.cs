...
public class ControlExtensionsTests
{
    ...
    [TestMethod]
    public void ControlTemplateTest1()
    {
        var expected = new ControlTemplate();
        var target = new ContentControl();
        Assert.AreSame(target, target.ControlTemplate(expected));
        var actual = target.Template;
        Assert.AreEqual(expected, actual);
    }

    [TestMethod, ExcludeFromCodeCoverage]
    public void ControlTemplateTest2()
    {
        var expected = () => new Grid();
        var target = new ContentControl();
        Assert.AreSame(target, target.ControlTemplate(expected));
        var actual = target.Template.VisualTree;
